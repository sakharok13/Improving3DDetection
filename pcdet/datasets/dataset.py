...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        elif self.dataset_cfg.TEST_AUGMENTOR.ENABLE:
            self.data_augmentor = DataAugmentor(
                self.root_path, self.dataset_cfg.TEST_AUGMENTOR, self.class_names, logger=self.logger
            )
        else:
    ...
    def prepare_data(self, data_dict, ssl_mode=False):
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
        ...
        if not self.training and self.data_augmentor is not None:  # TTA
            data_dict = self.data_augmentor.forward(
                data_dict={
                    **data_dict,
                }
