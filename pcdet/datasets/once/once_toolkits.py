...
class Octopus(object):
    ...
    def load_pcr_point_cloud(self, seq_id, frame_id):
        bin_path = osp.join(self.data_root, seq_id, 'mos_lidar_roof', '{}.bin'.format(frame_id))
        points = np.fromfile(bin_path, dtype=np.float16).astype('float32').reshape(-1, 4)
        return points
