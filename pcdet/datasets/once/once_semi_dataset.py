...
def split_once_semi_data(info_paths, data_splits, root_path, labeled_ratio, logger):
    ...
    for info_path in info_paths[train_split]:
        ...
        with open(info_path, 'rb') as f:
            ...
            once_pretrain_infos.extend(infos)
            once_labeled_infos.extend(infos)
    ...
    for info_path in info_paths[raw_split]:
        ...
        with open(info_path, 'rb') as f:
            ...
            once_unlabeled_infos.extend(infos)
...
class ONCESemiDataset(SemiDatasetTemplate):
    ...
    def get_pcr_lidar(self, sequence_id, frame_id):
        return self.toolkits.load_pcr_point_cloud(sequence_id, frame_id)
...
class ONCEUnlabeledDataset(ONCESemiDataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.dataset_cfg['PCR_TRAIN']:
            points = self.get_lidar(seq_id, frame_id)
            input_dict = {
                'points': points,
                'frame_id': frame_id,
            ...
            points_student = self.get_lidar(seq_id, frame_id)
            points_teacher = self.get_pcr_lidar(seq_id, frame_id)


            input_dict_teacher = {
                'points': points_teacher,
                'frame_id': frame_id,
            }
            if 'annos' in info:   # pseudo label
                annos = info['annos']
                input_dict_teacher.update({
                    'gt_names': annos['name'],
                    'gt_boxes': annos['boxes_3d'],
                })

            input_dict_student = {
                'points': points_student,
                'frame_id': frame_id,
            }
            if 'annos' in info:   # pseudo label
                annos = info['annos']
                input_dict_student.update({
                    'gt_names': annos['name'],
                    'gt_boxes': annos['boxes_3d'],
                })    


            teacher_dict, _ = self.prepare_data_ssl_unlabel(input_dict_teacher, output_dicts=['teacher'])
            _, student_dict = self.prepare_data_ssl_unlabel(input_dict_student, output_dicts=['student'])
