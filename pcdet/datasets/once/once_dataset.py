...
class ONCEDataset(DatasetTemplate):
    ...
    def class_balance(self, once_infos):

        _cls_infos = {name: [] for name in self.class_names}
    ...
    def include_once_data(self, split):
        ...
        # if self.split == 'train':
        #     once_infos = self.class_balance(once_infos)

        if self.use_val and self.training:
            once_infos_val = []
            info_path = str(info_path).replace('train', 'val')
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                once_infos_val.extend(infos)
            once_infos += once_infos_val
    ...
    def set_split(self, split):
        ...
        split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
        assert os.path.exists(split_dir), f"No such split found: {split_dir}"
    ...
    def get_infos(self, num_workers=4, sample_seq_list=None):
        ...
        assert sample_seq_list is not None, f"sample_seq_list is None and self.sample_seq_list={self.sample_seq_list}"
...
def create_once_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    ignore = ['train', 'val', 'test', 'raw_medium', 'raw_large']
