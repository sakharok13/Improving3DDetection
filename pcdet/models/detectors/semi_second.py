...
class SemiSECOND(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset, mode):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset, mode=mode)
        self.module_list = self.build_networks()
        self.model_type = None

        if 'ROI_HEAD' in model_cfg and mode=="student":
            if model_cfg.ROI_HEAD.NAME == 'BoxContrastHead':
                if model_cfg.ROI_HEAD.CONTRASTIVE_LOSS.ENABLE:
                    self.contrast_loss_func = loss_utils.ContrastiveLoss(
                        temperature=0.1
                    )
        ...
        #          (testing, return final_boxes)
            ...
            if self.training:
                if 'gt_boxes' in batch_dict: # for (pseudo-)labeled data
                    ...
                    if 'box_embedding' in batch_dict:  # for contrastive loss
                        contrast_loss = self.contrast_loss_func(batch_dict)
                        ret_dict.update({'contrastive_loss': contrast_loss})
