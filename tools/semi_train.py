...
def main():
    ...
    pretrain_model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=datasets['pretrain'], mode="original")
    ...
    if cfg.get('USE_PRETRAIN_MODEL', False):
        ...
        eval_pretrain_dir = output_dir / 'eval' / 'eval_with_pretraining'
        eval_pretrain_dir.mkdir(parents=True, exist_ok=True)
        args.start_epoch = cfg.OPTIMIZATION.PRETRAIN.NUM_EPOCHS - 2
        repeat_eval_ckpt(
    ...
    teacher_model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=datasets['labeled'], mode="teacher")
    """
    for param in teacher_model.parameters(): # ema teacher model
        param.detach_()
    """
    student_model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=datasets['labeled'], mode="student")
    ...
    if dist_train:
        ...
        student_model = nn.parallel.DistributedDataParallel(student_model, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()], find_unused_parameters=True)
        # teacher doesn't need dist train
        teacher_model = DistTeacher(teacher_model)
        teacher_model = nn.parallel.DistributedDataParallel(teacher_model, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()], find_unused_parameters=True)
