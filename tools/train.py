...
print(user_paths, ' !!! ETO USER PATHS')
import argparse
import datetime
import glob
import os
...
def main():
    ...
    model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=train_set,  mode="origin")
    if args.sync_bn:
    ...
    logger.info("MODEL DEVICE IS !!!!!!!!! = %s" % torch.cuda.device_count())
    logger.info(model)
    ...
    args.start_epoch = max(args.epochs - 5, 0)  # Only evaluate the last 10 epochs
