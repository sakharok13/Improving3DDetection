...
def iou_match_3d_filter(batch_dict, cfgs):
    ...
    for index in range(batch_size):
        ...
        # if not batch_dict['cls_preds_normalized'] and not "pred_dicts" in batch_dict:
        ...
        if not "pred_dicts" in batch_dict:
...
def iou_match_3d(teacher_model, student_model,
    ...
    gt_boxes, num_gt_list = construct_pseudo_label(teacher_boxes)
    ud_student_batch_dict['gt_boxes'] = gt_boxes

    # visualization(ud_student_batch_dict['points'], gt_boxes[0][:,:7], ud_student_batch_dict['frame_id'], 'IoU_match', None)
