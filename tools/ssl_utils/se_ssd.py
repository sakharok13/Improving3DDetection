...
def get_iou_consistency_loss(teacher_boxes, student_boxes):
    ...
    for teacher_box, student_box in zip(teacher_boxes, student_boxes):
        ...
        box_loss_reg = F.smooth_l1_loss(student_box_reg, matched_teacher_reg, reduction='none', beta=1.0 / 9.0)
        box_loss_reg = (box_loss_reg * matched_teacher_mask).sum() / num_matched_boxes
        box_loss_rot = F.smooth_l1_loss(torch.sin(student_box_rot - matched_teacher_rot), torch.zeros_like(student_box_rot), reduction='none', beta=1.0 / 9.0)
