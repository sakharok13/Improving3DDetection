...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        assert self.split in ['train', 'val', 'test', 'raw', 'train_D1', 'train_D2', 'train_D5', 'train_D10',
                              'train_D20']

        # if self.mode == 'train' and self.dataset_cfg.LABELED_INTERVAL.ENABLE:
        #     split_dir = self.root_path / 'ImageSets' / ('train' + '.txt')
        #     self.sample_sequence_list = [x.strip() for x in open(split_dir).readlines()]
        #     self.sample_sequence_list = sorted(self.sample_sequence_list)[::2]

        # self.ld_interval = int(self.dataset_cfg.LABELED_INTERVAL.INTERVAL.split('D')[-1])
        #     self.split_mode = self.dataset_cfg.SAMPLED_MODE[self.mode]
        #     if self.split_mode == 'sup':
        #         self.sample_sequence_list = sorted(self.sample_sequence_list)[::2]
            # elif self.split_mode == 'semi_sup':
            #     self.sample_sequence_unlabel = sorted(self.sample_sequence_list)[1::2]
            #     self.sample_sequence_list = sorted(self.sample_sequence_list)[::2]

        self.infos = []
        self.include_waymo_data(self.split)
    ...
    def set_split(self, split):
        ...
        self.split = split
        self.label_interval = None
        self.unlabel_interval = None
        if 'train' in self.split and 'D' in self.split:
            self.label_interval = int(self.split.split('_')[-1].split('D')[-1])
            self.split = 'train'
        elif self.split == 'raw':
            self.split = 'train'
            self.unlabel_interval = 1
        split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
        self.sample_sequence_list = [x.strip() for x in open(split_dir).readlines()]

        if self.label_interval is not None:
            self.sample_sequence_list = sorted(self.sample_sequence_list)[::2][::self.label_interval]
        elif self.unlabel_interval is not None:
            self.sample_sequence_list = sorted(self.sample_sequence_list)[1::2][::self.unlabel_interval]
        self.infos = []
    ...
    def include_waymo_data(self, mode):
        ...
        num_skipped_infos = 0
        # if mode=='train' and self.dataset_cfg.LABELED_INTERVAL.ENABLE:
        ...
        if self.dataset_cfg.SAMPLED_INTERVAL[mode] > 1:
            ...
            self.logger.info('Total sampled labeled samples for Waymo dataset: %d' % len(self.infos))

        # if mode == 'train' and self.split_mode == 'semi_sup':
        #     unlabel_infos = []
        #     # for k in range(len(self.sample_sequence_unlabel)):
        #     #     sequence_name = os.path.splitext(self.sample_sequence_unlabel[k])[0]
        #     #     info_path = self.data_path / sequence_name / ('%s.pkl' % sequence_name)
        #     #     # info_path = self.data_path / 'pseudo_label' / sequence_name / ('%s.pkl' % sequence_name)
        #     #     info_path = self.check_sequence_name_with_all_version(info_path)
        #     #     if not info_path.exists():
        #     #         num_skipped_infos += 1
        #     #         continue
        #     #     with open(info_path, 'rb') as f:
        #     #         infos = pickle.load(f)
        #     #         unlabel_infos.extend(infos)
        #
        #     info_path = self.data_path / '../' / 'waymo_infos_val_semi_even_D{}.pkl'.format(
        #         self.dataset_cfg.SAMPLED_INTERVAL['train'])
        #     with open(info_path, 'rb') as f:
        #         infos = pickle.load(f)
        #         unlabel_infos.extend(infos)
        #
        #     self.infos += unlabel_infos
        #     self.logger.info('Total sampled unlabled samples for Waymo dataset: %d' % len(unlabel_infos))
...
def create_waymo_infos_semi(dataset_cfg, class_names, data_path, save_path,
                            raw_data_tag='raw_data', processed_data_tag='waymo_processed_data_v0_5_0',
                            workers=min(16, multiprocessing.cpu_count())):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
    # train represents 399 scenes, and raw represents another 399 scenes.
    #     splits = ['train_D20', 'train_D10',]
    splits = ['train_D20', 'train_D10', 'train_D5', 'train_D2', 'train_D1', 'train', 'val', 'test', 'raw']
    ...
    for split in splits:
        ...
        filename = 'waymo_infos_%s.pkl' % split
        filename = save_path / Path(filename)
        dataset.set_split(split)
        waymo_infos = dataset.get_infos(
            raw_data_path=data_path / raw_data_tag,
            save_path=save_path / processed_data_tag, num_workers=workers, has_label=True,
            sampled_interval=1
        )
        ...
        print('Waymo info %s file is saved to %s' % (split, filename))
    ...
    for split in splits:
        ...
        if 'train' in split:
            filename = 'waymo_infos_%s.pkl' % split
            train_filename = save_path / filename

            dataset.set_split(split)
            dataset.create_groundtruth_database(info_path=train_filename, save_path=save_path, split=split,
                                                sampled_interval=1,
...
if __name__ == '__main__':
    ...
    elif args.func == 'create_waymo_infos_semi':
        create_waymo_infos_semi(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'waymo',
            save_path=ROOT_DIR / 'data' / 'waymo',
