...
class SSLDataAugmentor(object):
    ...
    def test_world_flip(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.test_world_flip, config=config)
        points = data_dict['points']
        points_all = [points]
        for cur_axis in config['ALONG_AXIS_LIST']:
            _, points_cur = getattr(augmentor_utils, 'test_flip_along_%s' % cur_axis)(
                None, points.copy(),
            )
            points_all.append(points_cur)
        data_dict['points'] = points_all
        data_dict['augmentation_params']['test_world_flip'] = config['ALONG_AXIS_LIST']
        return data_dict

    def test_world_rotation(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.test_world_rotation, config=config)
        ...
        for i in range(len(points)):
            for j in range(len(rot_range)):
                _, points_cur = augmentor_utils.global_rotation_test(
                    None, points[i].copy(), rot_range=rot_range[j]
                )
                points_all.append(points_cur)
        data_dict['points'] = points_all
        data_dict['augmentation_params']['test_world_rotation'] = rot_range
        return data_dict
