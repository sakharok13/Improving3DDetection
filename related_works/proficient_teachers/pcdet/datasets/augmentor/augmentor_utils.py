...
def test_flip_along_x(gt_boxes, points):
...
def test_flip_along_y(gt_boxes, points):
    if gt_boxes is not None:
        gt_boxes[:, 0] = -gt_boxes[:, 0]
...
def test_flip_along_xy(gt_boxes, points):
    if gt_boxes is not None:
        gt_boxes[:, 1] = -gt_boxes[:, 1]
        gt_boxes[:, 0] = -gt_boxes[:, 0]
        gt_boxes[:, -1] = gt_boxes[:, -1] + np.pi
...
def global_rotation_test(gt_boxes, points, rot_range):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
