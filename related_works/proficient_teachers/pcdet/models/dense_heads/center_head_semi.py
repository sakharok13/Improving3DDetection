...
class CenterHeadSemi(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.model_type == 'origin':
            ...
            data_dict = self.generate_predicted_boxes(data_dict)
            # data_dict['pred_cls_preds'] = [ele["pred_scores"] for ele in data_dict['pred_dicts']]
            # data_dict["pred_boxes"] = [ele["pred_boxes"] for ele in data_dict['pred_dicts']]
            # data_dict['pred_labels'] = [ele["pred_labels"] for ele in data_dict['pred_dicts']]
            data_dict['cls_preds_normalized'] = False
    ...
    def generate_predicted_boxes(self, data_dict):
        ...
        for task_id, pred_dict in enumerate(pred_dicts):
            ...
            # batch_hm = pred_dict['hm'].sigmoid_() inplace may cause errors
            batch_hm = torch.sigmoid(pred_dict['hm'])
            batch_reg = pred_dict['reg']
            batch_hei = pred_dict['height']
