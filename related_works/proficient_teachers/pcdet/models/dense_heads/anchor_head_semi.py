...
class AnchorHeadSemi(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, voxel_size, point_cloud_range,
        ...
        self.soft = self.model_cfg.get('SOFT_ASSIGNMENT', False)
        ...
        if self.model_type == 'origin':
            if self.training:
                targets_dict = self.assign_targets(
                    gt_boxes=data_dict['gt_boxes'], soft=False
                )
                ...
                if 'gt_boxes' in data_dict:
                    if not self.soft:
                        ...
                        targets_dict = self.assign_targets(
                            ...
                            soft=self.soft if (self.soft.ENABLE and data_dict['gt_boxes'].shape[-1] == 9) else False
                        )
                    self.forward_ret_dict.update(targets_dict)
