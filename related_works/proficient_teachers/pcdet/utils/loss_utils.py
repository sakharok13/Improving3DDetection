...
class ContrastiveLoss(nn.Module):
    ...
    def _dequeue_and_enqueue(self, keys):
        ...
        if ptr + keys.size(0) > self.K:
            keys = keys[:self.K - ptr]

        self.queue[:, ptr:ptr + keys.size(0)] = keys.T

        if ptr + keys.size(0) > self.K:
            ptr = 0
        else:
            ptr = (ptr + keys.size(0)) % self.K  # move pointer
    ...
    def forward(self, batch_dict):
        ...
        '''
        pos, neg = batch_dict['box_embedding']
        N = pos.size(0)

        z = batch_dict['box_memory']
        neg_other = torch.einsum('nc,ck->nk', [z, self.queue.clone().detach()])
        logits = torch.cat((pos, neg, neg_other), dim=1)
        logits /= self.temperature
        labels = torch.zeros((N,), dtype=torch.long).cuda()
        gpu_idx = random.randint(0, torch.distributed.get_world_size()-1)
        if z.device.index == gpu_idx:
