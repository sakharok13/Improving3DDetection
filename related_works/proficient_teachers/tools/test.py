...
def parse_config():
    ...
    parser.add_argument('--result_file', type=str, default=None, help='specify a result.pkl')
    ...
    parser.add_argument('--ssl_eval', action='store_true', default=False, help='visualization for pretrained model')
...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False):
    ...
    # start evaluation
    if args.ssl_eval:
        eval_utils.eval_one_epoch_ssl(
            cfg, model, test_loader, epoch_id, logger, dist_test=dist_test,
            result_dir=eval_output_dir, save_to_file=args.save_to_file, result_file=args.result_file
        )
    else:
...
def main():
    ...
    model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=test_set, mode="original")
    ...
    with torch.no_grad():
        if args.eval_all:
            # extra added
            args.start_epoch = max(cfg.OPTIMIZATION.NUM_EPOCHS - 5, 0)  # Only evaluate the last 10 epochs
